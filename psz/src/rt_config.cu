...
psz_error_status psz_query_device(psz_device_property* prop)
{
  ...
  // for fin-grain Histogram (p2013Histogram)
  {
    ...
    cudaDeviceGetAttribute(
        &prop->max_shmem_bytes, cudaDevAttrMaxSharedMemoryPerBlock, device_id);

    // The maximum optin shared memory per block. This value may vary by chip
    cudaDeviceGetAttribute(
        &prop->max_shmem_bytes_opt_in, cudaDevAttrMaxSharedMemoryPerBlockOptin,
        device_id);
  }
...
psz_error_status psz_launch_p2013Histogram(
    psz_device_property* prop, uint32_t* in_data, size_t const in_len,
    uint32_t* out_freq, int const num_buckets, float* milliseconds,
    void* stream)
{
  auto max_bytes = max(prop->max_shmem_bytes, prop->max_shmem_bytes_opt_in);

  cudaFuncSetAttribute(
      kernel::p2013Histogram<uint32_t, uint32_t>,
  ...
  int items_per_thread, r_per_block, grid_dim, block_dim, shmem_use;
  {
    ...
    // fits to size
    block_dim =
        ((((in_len / (grid_dim * items_per_thread)) + 1) / 64) + 1) * 64;
...
psz_error_status psz_hf_tune_coarse_encoding(
    size_t const len, psz_device_property* prop, int* sublen, int* pardeg)
{
...
int psz_hf_revbook_nbyte(int booklen, int symbol_bytewidth)
{
  ...
  return symbol_bytewidth * (2 * (symbol_bytewidth * 8)) +
         symbol_bytewidth * booklen;
}
