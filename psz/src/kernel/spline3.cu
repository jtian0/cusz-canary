    ...
    pszmem_cxx<T>* profiling_errors)
{
  ...
  if (intp_param.auto_tuning > 0) {
    ...
    if (intp_param.auto_tuning == 1) {
      ...
      cusz::c_spline_profiling_data<
          T*, SPLINE_DIM_3, PROFILE_BLOCK_SIZE_X, PROFILE_BLOCK_SIZE_Y,
          PROFILE_BLOCK_SIZE_Z, PROFILE_NUM_BLOCK_X, PROFILE_NUM_BLOCK_Y,
      ...
      if (l3.z != 1) {
        ...
        cusz::c_spline_profiling_data_2<
            T*, SPLINE_DIM_3, PROFILE_NUM_BLOCK_X, PROFILE_NUM_BLOCK_Y,
            PROFILE_NUM_BLOCK_Z, DEFAULT_BLOCK_SIZE>  //
            <<<auto_tuning_grid_dim, dim3(DEFAULT_BLOCK_SIZE, 1, 1), 0,
               (GpuStreamT)stream>>>(
                data->dptr(), data->template len3<dim3>(),
        ...
        intp_param.use_natural[0] = intp_param.use_natural[1] =
            intp_param.use_natural[2] = intp_param.use_natural[3] = do_nat;
        bool do_reverse = (errors[4 + do_nat] > 3 * errors[do_nat]);
        intp_param.reverse[0] = intp_param.reverse[1] = intp_param.reverse[2] =
            intp_param.reverse[3] = do_reverse;
        intp_param.use_md[0] = intp_param.use_md[1] = intp_param.use_md[2] =
            intp_param.use_md[3] = intp_param.use_md[4] =
        ...
        START_GPUEVENT_RECORDING(stream);
        cusz::c_spline_profiling_data_2<
            T*, SPLINE_DIM_2, PROFILE_NUM_BLOCK_X, PROFILE_NUM_BLOCK_Y, 1,
            DEFAULT_BLOCK_SIZE>  //
            <<<auto_tuning_grid_dim, dim3(DEFAULT_BLOCK_SIZE, 1, 1), 0,
               (GpuStreamT)stream>>>(
                data->dptr(), data->template len3<dim3>(),
      ...
      cusz::reset_errors<<<
          dim3(1, 1, 1), dim3(DEFAULT_BLOCK_SIZE, 1, 1), 0,
          (GpuStreamT)stream>>>(profiling_errors->dptr());
      ...
      if (l3.z > 1) {
        ...
        // best_error); printf("use_md[1] errors[11]=%f, best_error=%f\n",
        ...
        // best_error); printf("use_md[1] errors[17]=%f, best_error=%f\n",
      ...
      if (l3.z == 1) {
        ...
        // s_size_z); printf("l3.x = %d, .y = %d, .z = %d\n", l3.x, l3.y,
        // l3.z);
        cusz::pa_spline_infprecis_data<
            T*, float, LEVEL, SPLINE_DIM_2, AnchorBlockSizeX, AnchorBlockSizeY,
            AnchorBlockSizeZ, numAnchorBlockX, numAnchorBlockY,
        ...
        for (int level = 3; level < LEVEL; ++level) {
          ...
          intp_param.use_md[level_id] =
              (((best_idx + 3) % 6) == 2 or ((best_idx + 3) % 6) == 5);
          intp_param.reverse[level_id] = (best_idx + 3) % 3;
          best_ave_pre_error[level_id] =
              best_error / (calcnum(1 << level) * block_num);
          // printf(" BESTERROR=%f\n", best_ave_pre_error[level_id]);
        }
        // printf("\n");
      }
          ...
          cusz::pa_spline_infprecis_data<
              ...
              <<<dim3(s_size_x * s_size_y * s_size_z, 11, 1),
                 dim3(DEFAULT_BLOCK_SIZE, 1, 1), 0, (GpuStreamT)stream>>>(
                  ...
                  dim3(s_size_x, s_size_y, s_size_z),
                  dim3(S_STRIDE, S_STRIDE, S_STRIDE), eb_r, ebx2, intp_param,
                  profiling_errors->dptr(), false);
        else
          cusz::pa_spline_infprecis_data<
              T*, float, LEVEL, SPLINE_DIM_2, AnchorBlockSizeX,
    ...
    if(l3.z != 1){
      printf("\nNAT:");
      for(int i = 0; i < 4; ++i) printf(" %d", intp_param.use_natural[4 - i -
    1]); printf("\nMD:"); for(int i = 0; i < 4; ++i) printf(" %d",
    intp_param.use_md[4 - i - 1]); printf("\nREVERSE:"); for(int i = 0; i < 4;
    ++i) printf(" %d", intp_param.reverse[4 - i - 1]);
    }
    else{
      printf("\nNAT:");
      for(int i = 0; i < LEVEL; ++i) printf(" %d", intp_param.use_natural[LEVEL
    - i - 1]); printf("\nMD:"); for(int i = 0; i < LEVEL; ++i) printf(" %d",
    intp_param.use_md[LEVEL - i - 1]); printf("\nREVERSE:"); for(int i = 0; i <
    LEVEL; ++i) printf(" %d", intp_param.reverse[LEVEL - i - 1]);
    }
    */
  ...
  if (l3.z == 1) {
    ...
    cusz::c_spline_infprecis_data<
        T*, E*, float, LEVEL, SPLINE_DIM_2, AnchorBlockSizeX, AnchorBlockSizeY,
    ...
    cusz::c_spline_infprecis_data<
        T*, E*, float, 4, SPLINE_DIM_3, BLOCK16, BLOCK16, BLOCK16, 1, 1, 1,
        DEFAULT_BLOCK_SIZE>  //
        <<<grid_dim, dim3(DEFAULT_BLOCK_SIZE, 1, 1), 0, (GpuStreamT)stream>>>(
            data->dptr(), data->template len3<dim3>(),
            data->template st3<dim3>(),  //
