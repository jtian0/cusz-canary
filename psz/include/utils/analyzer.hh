...
class Analyzer {
 ...
 public:
  ...
  {
    if CONSTEXPR (
        policy == ExecutionPolicy::cuda_device and
        method == AnalyzerMethod::thrust) {
      auto t0 = hires::now();
      // ------------------------------------------------------------
      thrust::device_ptr<Data> g_ptr = thrust::device_pointer_cast(d_data);

      auto max_el_loc =
          thrust::max_element(g_ptr, g_ptr + len);  // excluding padded
      auto min_el_loc =
          thrust::min_element(g_ptr, g_ptr + len);  // excluding padded
      ...
      return extrema_result_t{
          max_val, min_val, rng, static_cast<duration_t>(t1 - t0).count()};
    }
    else {
      throw std::runtime_error(
          "Analyzer::get_maxmin_rng() Other policy and method not "
          "implemented.");
    }
  }
  ...
  static void get_histogram(
      UInt* data, size_t data_len, unsigned int* freq, size_t num_bins)
  {
    // TODO static check UInt
    if CONSTEXPR (
        policy == ExecutionPolicy::cuda_device and
        method == AnalyzerMethod::cuda_native) {
      float dummy;
      ...
      // TODO static check
      throw std::runtime_error(
          "Analyzer::get_histogram() using other policy or method not "
          "implemented.");
    }
  }

  Analyzer& estimate_compressibility_from_histogram(
      unsigned int* h_freq, size_t dict_size)
  {
    auto len =
        std::accumulate(h_freq, h_freq + dict_size, 0u);  // excluding outlier
    ...
    double r_lowerbound = 1 - (-std::log2(top1_prob) * top1_prob -
                               std::log2(1 - top1_prob) * (1 - top1_prob));
  ...
  Analyzer& get_stat_from_huffman_book(
      const unsigned int* h_freq, const Huff* h_codebook, size_t len,
      size_t num_bins)
  {
  ...
  Analyzer& print_compressibility(
      bool print_huffman_stat = false, bool print_dropout = false,
      double equiv_origin_bitlen = 32)
  {
    ...
    if (theory.hist.top1_prob < 0.4) {
      ...
      cout << "* Compressibility upper bound is determined by the lower bound "
              "of average bitlength."
           << '\n';
      cout << "  - est. redundancy lower bound (p1 > 0.4):\t"
           << theory.huffman_theory.r_lowerbound << '\n';
      cout << "  - est. avg.bitlen lower bound (p1 > 0.4):\t"
           << theory.huffman_theory.avgb_lowerbound << '\n';
      ...
      if (print_dropout) {
        ...
        auto dropout_equiv_bitlen_1_5x =
            theory.hist.dropout_equiv_bitlen_1_5x();
        ...
        cout << "    | reduction rate:\t"
             << (equiv_origin_bitlen / dropout_equiv_bitlen_2x) << '\n';
        cout << "    | bitlen_dropout <= bitlen_enc?\t"
             << (dropout_equiv_bitlen_2x <=
                 theory.huffman_theory.avgb_lowerbound)
             << '\n';
        cout << "  - dropout at 0.5x metadata overhead" << '\n';
        cout << "    | equiv.bitlen:\t" << dropout_equiv_bitlen_1_5x << '\n';
        cout << "    | reduction rate (fp32):\t"
             << (equiv_origin_bitlen / dropout_equiv_bitlen_1_5x) << '\n';
        cout << "    | bitlen_dropout <= bitlen_enc?\t"
             << (dropout_equiv_bitlen_1_5x <=
                 theory.huffman_theory.avgb_lowerbound)
             << '\n';
        cout << '\n';
      }
