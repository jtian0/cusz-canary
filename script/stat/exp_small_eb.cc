...
#pragma omp parallel for shared(len, compute) private(i, p_ebx2_r, p_ebx2)
      for (i = 0; i < len; i++)
        compute[i] = std::round(compute[i] * ebx2_r) * ebx2;
    }
    auto z = hires::now();
    cout << "dryrun time: " << static_cast<duration_t>(z - a).count()
         << " sec\n";
  }
...
int main(int argc, char** argv)
{
  if (argc < 4)
    throw std::runtime_error(
        "./prog fname len eb <input type> <compute type> <if r2r>");
  ...
  if (argc == 7)
    if_r2r = std::string(argv[5]) == "r2r" or std::string(argv[5]) == "true";

  if (argc >= 6) {
    if (std::string(argv[4]) == "float" and std::string(argv[5]) == "float")
      f<float, float>(if_r2r);
    else if (
        std::string(argv[4]) == "float" and std::string(argv[5]) == "double")
      f<float, double>(if_r2r);
    else if (
        std::string(argv[4]) == "double" and std::string(argv[5]) == "double")
      f<double, double>(if_r2r);
    else if (
        std::string(argv[4]) == "double" and std::string(argv[5]) == "float")
